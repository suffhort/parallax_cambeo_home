$(document).ready(function() {
	...
	$('a.sectionLevels').click(function(){
    	$('html, body').animate({
    		scrollTop:$('#sectionLevels').offset().top
    	}, 1000, function() {
	    	parallaxScroll(); // Callback is required for iOS
		});
    	return false;
...
function redrawDotNav(){
	...
	var section3Top =  $('#english-channel').offset().top - (($('#sectionLevels').offset().top - $('#english-channel').offset().top) / 2);
	var section4Top =  $('#sectionLevels').offset().top - (($(document).height() - $('#sectionLevels').offset().top) / 2);;
	...
	if($(document).scrollTop() >= section1Top && $(document).scrollTop() < section2Top){
		...
		$('nav#primary a.sectionLevels').addClass('active');
	}
